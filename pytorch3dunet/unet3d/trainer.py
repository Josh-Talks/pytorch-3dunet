...
class UNetTrainer:
    ...
    def __init__(
        ...
        max_num_epochs,
        max_num_iterations,
        validate_after_iters_initial=50,
        validate_after_iters_final=500,
        validate_switch_iteration=750,
        log_after_iters=100,
        validate_iters=None,
        num_iterations=1,
        num_epoch=0,
        ...
        **kwargs,
        ...
        self.writer = SummaryWriter(
            log_dir=os.path.join(
                checkpoint_dir, "logs", datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            )
        )

        assert (
            tensorboard_formatter is not None
        ), "TensorboardFormatter must be provided"
