...
from pytorch3dunet.unet3d.se import (
    ChannelSELayer3D,
...
def create_conv(
    ...
    for i, char in enumerate(order):
        if char == "r":
            ...
            raise ValueError(
                f"Unsupported layer type '{char}'. MUST be one of ['b', 'g', 'i', 'r', 'l', 'e', 'c', 'd', 'D']"
            )
        ...
        if upsample is not None and upsample != "none":
            if upsample == "default":
                if basic_module == DoubleConv:
                    upsample = (
                        "nearest"  # use nearest neighbor interpolation for upsampling
                    )
...
class TransposeConvUpsampling(AbstractUpsampling):
    ...
    def __init__(
        self, in_channels, out_channels, kernel_size=3, scale_factor=2, is3d=True
    ):
